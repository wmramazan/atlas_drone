#include "DroneNavigation/BlockAllocator.h"
#include <limits.h>
#include <memory.h>
#include <stddef.h>
#include <malloc.h>
...
int BlockAllocator::block_sizes_[BlockSizes] =
{
    16,     // 0
    32,     // 1
    64,     // 2
...
uint8_t BlockAllocator::s_block_size_lookup_[MaxBlockSize + 1];

BlockAllocator::BlockAllocator()
{
    assert(BlockSizes < UCHAR_MAX);

    num_chunk_space_ = ChunkArrayIncrement;
    num_chunk_count_ = 0;
...
void* BlockAllocator::Allocate(int size)
{
    ...
    else
    {
        if (num_chunk_count_ == num_chunk_space_)
        {
            ...
            memset(chunks_ + num_chunk_count_, 0, ChunkArrayIncrement * sizeof(Chunk));
            ::free(oldChunks);
        }
        ...
        int block_count = ChunkSize / block_size;
        assert(block_count * block_size <= ChunkSize);
